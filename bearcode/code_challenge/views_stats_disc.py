...
def random_pick(request):
    ...
    if not shuffle_problems:
        ...
        return render(request, 'code_challenge/global_stream.html',
                      {'problems': problems, 'currentuser': request.user})
...
def search_discussion_page(request):
    """
    The discussions page where all the discussions displayed and user can search for specific
    discussion
    :param request: the request to display discussions available
    :return: a dict containing the status and the message
    """
    discussions = Discussion.objects.all().order_by('-created_at')
    return render(request, 'code_challenge/search_discussion.html',
                  {'discussions': discussions, 'currentuser': request.user})
...
def search_discussion(request):
    ...
    user_input = request.GET['userInput'].lower()
    ...
    for discussion in candidates:
        if user_input in discussion.title.lower() or user_input in discussion.text.lower():
